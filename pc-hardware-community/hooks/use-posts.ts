...
export function useUpdatePost() {
  ...
  return useMutation({
    ...
    onSuccess: (_: any, variables: { id: number; data: Partial<Post> }) => {
      // 관련 캐시 무효화
      queryClient.invalidateQueries({ queryKey: ['posts'] })
...
export function useToggleLike() {
  ...
  return useMutation({
    ...
    onSuccess: (data: { data: { likesCount: number } }, postId: number) => {
      // 포스트 상세 정보 업데이트
...
export function useIncrementViews() {
  ...
  return useMutation({
    ...
    onSuccess: (data: { data: { viewsCount: number } }, postId: number) => {
      // 포스트 상세 정보 업데이트
      queryClient.setQueryData(['post', postId], (old: any) => {
        if (old?.data) {
          return {
...
export function useCreateComment() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      content,
      parentCommentId,
    }: { postId: number; content: string; parentCommentId?: number }) =>
      commentApi.createComment(postId, content, parentCommentId),
    onSuccess: (
      _: unknown,
      { postId }: { postId: number; content: string; parentCommentId?: number }
    ) => {
...
export function useUpdateComment() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ commentId, content }: { commentId: number; content: string }) => commentApi.updateComment(commentId, content),
    onSuccess: (_: unknown, { postId }: { postId: number; content: string }) => queryClient.invalidateQueries({ queryKey: ['comments', postId] }),
  })
}
